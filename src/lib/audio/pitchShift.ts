 ...
 */
export function pitchShift(audio: Float32Array, semitones: number): Float32Array {
...
function cubicInterpolate(s0: number, s1: number, s2: number, s3: number, t: number): number {
  const t2 = t * t;
  const t3 = t2 * t;
